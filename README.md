...
- **180° flip disambiguation** — `correct_coarse_orientation()` guarantees a
  ...
  ambiguous between the two landscape orientations). `IDCardOCR.extract_best_orientation()`
  runs OCR on both the image and its 180° rotation, and picks whichever yields
...
- **Field extraction** — uses Arabic field-label anchors (`الاسم`, `العنوان`,
  `الرقم القومي`) to locate the Name, Address, and National ID lines relative to
  their labels, with heuristic fallbacks (longest Arabic line, 14-digit regex scan)
  when labels aren't detected cleanly. Address collection automatically stops at
  the next field's label so it doesn't swallow unrelated text.
...
Example output on a small (n=3) synthetic test set:
